...
def main():
	...
	*********************************************************************
	...
	def	model_with_sol(subset,yp_sol,yr_sol):
		...
		return obj,xp_sol,xr_sol, sp_sol,sr_sol, yp_sol,yr_sol, timer(start)


	obj,xp_sol,xr_sol, sp_sol,sr_sol, yp_sol,yr_sol,temp = model_with_sol(subset,yp_sol,yr_sol)

		
	arquivo = open('clsr_relax_table'+str(fator)+'.txt','a')
	arquivo.write(file_name+';'+str(round(obj,3))+';'+str(temp)+'\n')
	arquivo.close()
