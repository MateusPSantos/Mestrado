void lerdados (FILE *fp, double *HP, double *PP, double *FP, double *HR, double *PR, double *FR, double *D, double *R, double setup) {
  ...
  if (opc.instance_mathijn == 1) {
    ...
    for (i=0; i<11; i++) {
      //fgets(mystring[i], 200, fp);
    }
    
    for (i=0; i<N; i++)	{
      if (fscanf(fp, "%d", &(t)) != 1) {
	fprintf(stderr, "error : falta entrada, 1 math\n");
	exit(1);
      }
      t -= 1;
      
      if (fscanf(fp, "%lf", &(D[t])) != 1) {
	fprintf(stderr, "error : falta entrada, 2 math \n");
	exit(1);
      }
      
      if (fscanf(fp, "%lf", &(R[t])) != 1) {
	fprintf(stderr, "error : falta entrada, 3 math \n");
	exit(1);
      }
      
      //fgets(mystring[i+10], 200, fp);
